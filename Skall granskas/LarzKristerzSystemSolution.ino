/**
   ...
   State machine for the whole system
...
void loop()
{
  ...
  else if (twi_cmd_t == TWI_CMD_PICK_UP_STATUS && pick_up_status_t != PICK_UP_DONE && flagDrive == 1)
  {
    ...
    /* The crane will wait while Arlo is moving forward */
    state = Wait;
  ...
  switch (state)
  {
    case SensorDetection:
      ...
      if (distance_cm <= 40 && distance_cm >= 30 ) 
      {
        ...
        state = Wait;
        detectionTries = 0;
        /*This flag will make sure hat arlo will go forward only if the object is detected*/
        flagDrive = 1;
        Serial.println("GOING FORWARD!");
      }
      else
      {
        /* If the object cannot be found Arlo is informed about it so it can rotate 10 degrees*/
        if (detectionTries == 5)
        {
          state = Wait;
          detectionTries = 0;

          /* Arlo will send command TWI_CMD_PICK_UP_START when it has rotated 10 degrees */
    ...
    case DropObject:
      ...
      break;
    case Wait:
      //Do nothing while waiting for Arlo
      break;
  }
}
...
/****************************************************************/
/*               Methods for ultrasonicsensor                   */
/****************************************************************/

/* Method that returns distance to object*/
